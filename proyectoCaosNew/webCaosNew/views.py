...
def buscar_noticia(request):
    ...
    print(desc)
    notic = Noticia.objects.filter(publicar=True).filter(titulo_noticia__contains=desc)
    contar = Noticia.objects.filter(publicar=True).filter(titulo_noticia__contains=desc).count()
    print(contar)
...
def filtro_descripcion(request):
    ...
    print(desc)
    notic = Noticia.objects.filter(publicar=True).filter(nombre_periodista__contains=desc)
    print(notic)
    contar = Noticia.objects.filter(publicar=True).filter(nombre_periodista__contains=desc).count()
    print(contar)
    contexto={'noticia':notic,'cantidad':contar,'categorias':categorias}
    return render(request,"galeria.html",contexto)
